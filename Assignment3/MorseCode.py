...
def Build_Character_Encoder():
    ...
    Character_Encoder = dict(zip(CharList1, CodeList1))
    #number characters
    Number_Encoder = {}
    Number_Encoder = dict(zip(CharList2, CodeList2))
    #update two dictionary
    Character_Encoder.update(Number_Encoder)
    #special character
    Special_Encoder = {}
    Special_Encoder = dict([(key, '.-.-.-') for key in CharList3])
    
    Character_Encoder.update(Special_Encoder)
...
def Build_Character_Decoder():
    ...
    Character_Decoder = {}
    # add your code from here
    Character_Decoder = dict(zip(CodeList1, CharList1))
    #number characters
    Number_Decoder = {}
    Number_Decoder = dict(zip(CodeList2, CharList2))
    #update two dictionary
    Character_Decoder.update(Number_Decoder)
    #special character
    Special_Decoder = {'.-.-.-':'#'}
    Character_Decoder.update(Special_Decoder)
...
def Word_Encoder(Word, Character_Encoder=None):
    ...
    for i, c in enumerate(Word):
        List.append(Character_Encoder.get(c))

    Word_in_Morse_Code = ' '.join(List)
...
def Word_Decoder(Word_in_Morse_Code, Character_Decoder=None):
    ...
    MorseList = Word_in_Morse_Code.split(' ')
    
    for i, c in enumerate(MorseList):
        Word += Character_Decoder.get(c)
...
def Message_Encoder(Message, Character_Encoder=None):
    ...
    while i< len(MessageList) :
        List.append(Word_Encoder(MessageList[i]))
        
        i += 1
    Message_in_Morse_Code = ','.join(List)
...
def Message_Decoder(Message_in_Morse_Code, Character_Decoder=None):
    ...
    Message = ''
    List = []
    MessageList = Message_in_Morse_Code.split(',')
    
    i = 0
    while i < len(MessageList) :
        List.append(Word_Decoder(MessageList[i]))
        i += 1
